...
export function LevelCard({ level, totalPoints }: { level: LevelInfo; totalPoints: number }) {
  return (
    <div
      className="mb-8 flex flex-col gap-3 rounded-3xl border border-black/5 bg-white p-6"
      style={{ background: "linear-gradient(135deg, color-mix(in srgb, var(--primary) 8%, white), white)" }}
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span
            className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-xl font-extrabold text-white shadow-md"

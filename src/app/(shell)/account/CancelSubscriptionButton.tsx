      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-6" onClick={() => setConfirmOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-xl">
            ...
            <h2 className="mt-2 text-lg font-bold">{t("account.cancel.confirmTitle")}</h2>
            <p className="mt-2 text-sm opacity-70">
              {t("account.cancel.confirmBodyPrefix")} <span className="font-semibold">{periodEndLabel}</span> {t("account.cancel.confirmBodySuffix")}
            </p>
            ...
            <div className="mt-5 flex gap-2">
              ...
              >
                {pending ? t("account.cancel.canceling") : t("account.cancel.yesCancel")}
              </button>
            </div>
          </div>
        </div>
